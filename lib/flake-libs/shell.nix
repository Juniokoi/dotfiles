...
let
  ...
  user-shells-root = flake-libs.fs.get-file "shells";
in
{
  shell = {
    ...
    create-shells =
      ...
      let
        user-shells = flake-libs.fs.get-default-nix-files-recursive src;
        create-shell-metadata = shell:
          {
            name = builtins.unsafeDiscardStringContext (flake-libs.path.get-parent-directory shell);
