...
rec {
  flake = rec {
    ...
    #   result: { x = true; }
    without-snowfall-inputs = flake-libs.fp.compose without-self without-src;
  ...
  mkFlake = full-flake-options:
    let
      ...
      systems = flake-libs.system.create-systems (full-flake-options.systems or { });
      hosts = flake-libs.attrs.merge-shallow [ (full-flake-options.systems.hosts or { }) systems ];
      templates = flake-libs.template.create-templates {
        overrides = (full-flake-options.templates or { });
      };
      modules = flake-libs.module.create-modules {
        overrides = (full-flake-options.modules or { });
      };
      overlays = flake-libs.overlay.create-overlays {
      ...
      outputs-builder = channels:
        let
          ...
          packages = flake-libs.package.create-packages {
            inherit channels;
            overrides = (full-flake-options.packages or { }) // (user-outputs.packages or { });
          };
          shells = flake-libs.shell.create-shells {
            inherit channels;
        ...
        in
        flake-libs.attrs.merge-deep [ user-outputs outputs ];

      flake-options = custom-flake-options // {
        inherit hosts templates;
        inherit (user-inputs) self;

        lib = flake-libs.internal.user-lib;
        inputs = flake-libs.flake.without-src user-inputs;

        nixosModules = modules;

        channelsConfig = full-flake-options.channels-config or { };

        channels.nixpkgs.overlaysBuilder = flake-libs.overlay.create-overlays-builder {
          overlay-package-namespace = full-flake-options.overlay-package-namespace or null;
