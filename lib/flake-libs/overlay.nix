...
let
  ...
  user-overlays-root = flake-libs.fs.get-file "overlays";
...
in
{
  overlay = {
    ...
    create-overlays-builder =
      ...
      let
        user-overlays = flake-libs.fs.get-default-nix-files-recursive src;
        create-overlay = overlay: import overlay (user-inputs // { inherit channels; });
        user-packages-overlay = final: prev:
          let
            user-packages = flake-libs.package.create-packages {
              pkgs = final;
              channels = channels;
            };
    ...
    create-overlays =
      ...
      let
        ...
        create-overlay = (overlays: file:
          let
            name = builtins.unsafeDiscardStringContext (flake-libs.path.get-parent-directory file);
            overlay = final: prev:
              let
                channels = channel-systems.${prev.system};
        ...
        user-packages = flake-libs.fs.get-default-nix-files-recursive packages-src;
        ...
        default-overlay = final: prev:
          let
            ...
            merged-results = flake-libs.attrs.merge-shallow-packages
