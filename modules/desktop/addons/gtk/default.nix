...
with lib.internal;
let
  cfg = config.koi.desktop.addons.gtk;
  gdmCfg = config.services.xserver.displayManager.gdm;
in
{
  options.koi.desktop.addons.gtk = with types; {
    ...
    cursor = {
      ...
      pkg = mkOpt package pkgs.koi.bibata-cursors "The package to use for the cursor theme.";
    };
    icon = {
