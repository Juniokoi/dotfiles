...
let
  ...
  user = config.users.users.${config.settings.user.name};
  ...
  other-hosts = lib.filterAttrs
    (key: host:
      key != name && (host.config.settings.user.name or null) != null)
  ...
  other-hosts-config = lib.concatMapStringsSep
    ...
    (name:
      let
        ...
        remote-user-name = remote.config.settings.user.name;
...
in
{
  options.settings.services.openssh = with types; {
  ...
  config = mkIf cfg.enable {
    ...
    settings.user.extraOptions.openssh.authorizedKeys.keys =
      cfg.authorizedKeys;
