...
let
  ...
  shares-submodule = with types; submodule ({ name, ... }: {
    options = {
      ...
      only-owner-editable = mkBoolOpt false "Whether the share is only writable by the system owner (koi.user.name).";
...
in
{
  options.koi.services.samba = with types; {
    ...
    services.samba = {
      ...
      shares = mapAttrs
        (name: value: {
          ...
          "write list" = config.koi.user.name;
