...
with lib.internal;
let
  cfg = config.koi.services.openssh;

  user = config.users.users.${config.koi.user.name};
  ...
  other-hosts = lib.filterAttrs
    (key: host:
      key != name && (host.config.koi.user.name or null) != null)
  ...
  other-hosts-config = lib.concatMapStringsSep
    ...
    (name:
      let
        ...
        remote-user-name = remote.config.koi.user.name;
...
in
{
  options.koi.services.openssh = with types; {
  ...
  config = mkIf cfg.enable {
    ...
    koi.user.extraOptions.openssh.authorizedKeys.keys =
      cfg.authorizedKeys;
